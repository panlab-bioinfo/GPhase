...
def run_partig(asm_fa, partig_k, partig_w, partig_c, partig_m, output_prefix,logger):
    execute_command(f"samtools faidx {asm_fa}", "Failed to indexing the assembly FASTA",logger)
...
def convert_partig_output(asm_fa, partig_k, partig_w, partig_c, partig_m, output_prefix,logger):
    ...
    execute_command(
        f"python {script_path_add} -fai {asm_fa}.fai -p {output_prefix}.partig.{partig_k}_{partig_w}_{partig_c}_{partig_m}.txt -o {output_prefix}.partig.{partig_k}_{partig_w}_{partig_c}_{partig_m}.csv", "Converting Partig output to CSV",logger)
...
def main():
    ...
    # Step 1: Run partig
    run_partig(args.asm_fa, args.partig_k, args.partig_w, args.partig_c, args.partig_m, args.output_prefix,logger)
    convert_partig_output(args.asm_fa, args.partig_k, args.partig_w, args.partig_c, args.partig_m, args.output_prefix,logger)
